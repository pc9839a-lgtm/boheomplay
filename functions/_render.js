...
export function renderCategoryPage(category) {
  ...
  const body = `<main><section class="content-hero"><div class="wrap">${breadcrumb([{name:category.name}])}<span class="eyebrow">CATEGORY HUB</span><h1>${esc(category.name)} 질문 모음</h1><p>${esc(description)}</p></div></section><section class="section"><div class="wrap content-layout"><article class="article-card"><h2>${esc(category.name)} 상담 전 확인할 것</h2><p>${esc(category.description)}</p><p>아래 질문들은 검색 유입과 상담 전환을 동시에 고려해 구성된 페이지입니다. 각 질문 페이지에서 요약, 체크리스트, 주의사항을 확인할 수 있습니다.</p><div class="list-grid">${items.map((item) => `<a href="/q/${item.slug}">${esc(item.title)}<small>${esc(item.keyword)} · ${esc(item.audience)}</small></a>`).join('')}</div></article><aside class="side-card"><h3>${esc(category.name)} 상담 신청</h3><p>질문을 읽어도 내 상황에 맞는 판단이 어렵다면 상담 신청으로 현재 조건을 남겨주세요.</p><a class="btn btn-primary" href="/#contact?topic=${topicQuery(category.topic)}">상담 신청하기</a></aside></div></section></main>`;
  return layout({ title, description, path, body, schema });
}
