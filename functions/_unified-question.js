...
</style>${schema.map((value) => `<script type="application/ld+json">${safeJson(value)}</script>`).join('')}</head><body><header class="site-header"><div class="wrap header-inner"><a class="brand" href="/">${esc(SITE.name)}</a><button class="menu-btn" type="button" aria-label="메뉴 열기" data-menu-toggle><span></span><span></span><span></span></button><nav class="nav" data-nav><a href="/#board">질문게시판</a><a href="/#write">질문남기기</a></nav></div></header><main class="thread-page"><section class="thread-view"><div class="wrap thread-wrap"><div class="thread-breadcrumb"><a href="/">홈</a> / <a href="/#board">질문게시판</a> / ${esc(item.category || '보험정보')}</div><section class="question-post"><div class="thread-meta"><span>${esc(item.category || '보험정보')}</span><span>익명</span><span>업데이트 ${esc(updatedAt)}</span></div><h1>${esc(item.title)}</h1><div class="question-body">${nl(item.question)}</div>${companyLink}<div class="thread-actions"><div class="thread-actions-left"><span>♡ 0</span><span>💬 1</span></div><div class="thread-actions-right"><span>공유</span><span>저장</span></div></div></section><div class="answer-post"><div class="answer-avatar">보</div><article><div class="answer-meta"><strong>${esc(SITE.name)} 답변</strong><span>업데이트 ${esc(updatedAt)}</span></div><div class="answer-content"><p>${nl(item.lead)}</p><div class="answer-point">${esc(item.point)}</div><div class="answer-subtitle">먼저 봐야 할 부분</div><ul>${(item.bullets || []).map((bullet) => `<li>${esc(bullet)}</li>`).join('')}</ul><p>${nl(item.close)}</p>${sourceBlock}</div></article></div><section class="thread-related"><h2>관련 질문</h2><div class="thread-related-list">${related.map((entry) => `<a href="/q/${esc(entry.slug)}">${esc(entry.title)}</a>`).join('')}</div></section></div></section></main><footer class="footer"><div class="wrap footer-inner"><p><strong>${esc(SITE.company)}</strong> · 대표 ${esc(SITE.owner)} · 사업자번호 ${esc(SITE.businessNumber)}</p><nav><a href="/insurance-notice/">보험정보 이용안내</a><a href="/privacy/">개인정보처리방침</a><a href="/terms/">이용약관</a></nav></div></footer><script src="/assets/js/main.js?v=20260715-unified-question-v2"></script></body></html>`;
}
