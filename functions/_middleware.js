...
const BOARD_UI='<script src="/assets/js/board-ui-restore.js?v=20260715-board-restore-v12"></script>';
...
function patchHtml(html){
  let out=html
    ...
    .replace(/<script[^>]+src=["'][^"']*\/assets\/js\/board-ui-restore\.js[^"']*["'][^>]*><\/script>/g,'')
    .replace(/\/assets\/js\/config\.js\?v=[^"']+/g,'/assets/js/config.js?v=20260715-board-restore-v12')
    .replace(/\/assets\/js\/main\.js\?v=[^"']+/g,'/assets/js/main.js?v=20260715-board-restore-v12');
  ...
  if(out.includes('</body>')) out=out.replace('</body>',`${BOARD_UI}</body>`);
  return out;
}
