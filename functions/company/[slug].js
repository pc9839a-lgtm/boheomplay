...
function addRelatedQuestions(html, company) {
  const related = dailyQuestions20260715
    .concat(dailyQuestions20260714)
    .concat(dailyQuestions20260713)
    .filter((item) => item.cs === company.slug);
  if (!related.length) return html;

  const latestDate = related.some((item) => dailyQuestions20260715.includes(item))
    ? '2026-07-15'
    : related.some((item) => dailyQuestions20260714.includes(item))
      ? '2026-07-14'
      : '2026-07-13';
