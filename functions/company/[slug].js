...
function addRelatedQuestions(html, company) {
  const related = dailyQuestions20260713.filter((item) => item.cs === company.slug);
  if (!related.length) return html;

  const section = `<section class="directory-section"><h2>${esc(company.name)} 관련 계약관리 질문</h2><p>회사명과 공식 홈페이지는 2026-07-13 생명보험협회 회원사 안내와 회사 공식 사이트에서 확인했습니다.</p><div class="directory-grid">${related.map((item) => `<a class="directory-card" href="/q/${esc(item.slug)}"><strong>${esc(item.title)}</strong><span>${esc(item.q.join(' '))}</span></a>`).join('')}</div></section>`;
